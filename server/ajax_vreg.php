...
try {
    ...
    $totalData = $conn->query("SELECT count(*) FROM vendor_reg")->fetchColumn();
    $totalFiltered = $totalData;

    if (isset($_REQUEST['search']['value'])) {
        $search_value = $_REQUEST['search']['value'];

        $sql .= " WHERE vendor_type LIKE '%" . $search_value . "%'";
...
foreach ($result as $row) {
    ...
    $object->vreg_id    = $id;
    $object->vendor_type  = $row['vendor_type'];
    $object->documents  = $row['documents'];
    $object->qms  = $row['qms'];
