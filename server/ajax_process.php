...
try {
    ...
    $totalData = $conn->query("SELECT count(*) FROM process")->fetchColumn();
    $totalFiltered = $totalData;

    if (isset($_REQUEST['search']['value'])) {
        $search_value = $_REQUEST['search']['value'];

        $sql .= " WHERE p_name LIKE '%" . $search_value . "%'";
...
foreach ($result as $row) {
    ...
    $object->process_id    = $id;
    $object->p_name  = $row['p_name'];
    $object->short_name  = $row['short_name'];
