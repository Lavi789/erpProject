...
try {
    ...
    $sql .= " ORDER BY " . " CREATED_AT  DESC " . "  LIMIT " . $_REQUEST['start'] . " ," . $_REQUEST['length'] . "   ";
...
// </a>';
    ...
    $object->stock_id   = $id;
    $object->item_code  = $row['item_code'];

    
    
    $object->batch_no  = $row['batch_no'];
    $object->exp_date  = $row['exp_date'];
    $object->qty  = $row['qty'];
