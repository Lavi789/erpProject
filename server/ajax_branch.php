...
try {
    ...
    $sql = "SELECT br.*, bank_name FROM branch br
            INNER JOIN bank ba ON br.bank_id = ba.bank_id";

    $totalData = $conn->query("SELECT count(*) FROM branch")->fetchColumn();
    $totalFiltered = $totalData;

    if (isset($_REQUEST['search']['value'])) {
        $search_value = $_REQUEST['search']['value'];

        $sql .= " WHERE branch_name LIKE '%" . $search_value . "%'";
...
foreach ($result as $row) {
    ...
    $object->bank_name      = $row['bank_name'];
