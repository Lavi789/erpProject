...
try {
    ...
    $totalData = $conn->query("SELECT count(*) FROM shift")->fetchColumn();
    $totalFiltered = $totalData;

    if (isset($_REQUEST['search']['value'])) {
        $search_value = $_REQUEST['search']['value'];

        $sql .= " WHERE shift LIKE '%" . $search_value . "%'";
...
foreach ($result as $row) {
    ...
    // $object->in_time  = $row['in_time'];
    // $object->out_time  = $row['out_time'];
    // $object->deduction  = $row['deduction'];
    // $object->grace  = $row['grace'];
