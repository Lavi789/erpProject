...
$supplier = $conn->query("SELECT supplier_id, supplier_name FROM supplier ORDER BY supplier_name ASC")->fetchAll(PDO::FETCH_ASSOC);
?>
...
<body class="py-5 md:py-0">
    ...
    <div class="content content--top-nav">
        ...
        <div class="mt-8">
            ...
            <a href="hindalco/tcpdf/invoice.php">
...
<script>
    ...
    function updateAmount(count) {
        const qty = $("#inp_qty" + count).val();
        const purchaseRate = parseFloat($("#inp_purchase_rate" + count).val());
        const discPer = parseFloat($("#inp_disc_per" + count).val());
        const discAmtOld = parseFloat($("#inp_disc_amt" + count).html());
        let discRate = parseFloat($("#inp_disc_rate" + count).html());
        const amountOld = parseFloat($("#inp_amount" + count).html());
        const netAmtOld = parseFloat($("#inp_net_amt" + count).html());
        const gstFlag = $("#inp_gst_flag" + count).html();
        const gstPer = parseFloat($("#inp_gst_per" + count).html());
        const gstAmtOld = parseFloat($("#inp_gst_amt" + count).html());
        let gstRate = parseFloat($("#inp_gst_rate" + count).html());
        const salesRate = parseFloat($("#inp_sales_rate" + count).val());
        const batchNo = $("#inp_batch_no" + count).val();

        // If sales rate is not entered show popup
        if (!salesRate) {
            Swal.fire({
                text: 'Please enter sales rate!!!',
                icon: "info"
            });
        ...
        // If batch no is not entered show popup
        if (!batchNo) {
            Swal.fire({
